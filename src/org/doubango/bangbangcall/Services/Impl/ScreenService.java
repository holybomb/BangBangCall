...
*/
package org.doubango.bangbangcall.Services.Impl;
...
import org.doubango.bangbangcall.BangBangCall;
import org.doubango.bangbangcall.Main;
...
public class ScreenService extends NgnBaseService implements IScreenService {
	...
	public boolean back() {
		...
		if(mLastScreensIndex == 0){
			if((screen = mLastScreens[mLastScreens.length-1]) == null){
				...
				{
					Engine.getInstance().getMainActivity().startActivity(new Intent(Engine.getInstance().getMainActivity(), ScreenLogin.class));
					return false;
				}
				return show(ScreenTabDialer.class);
			}
			else{
	...
	public boolean bringToFront(int action, String[]... args) {
		Intent intent = new Intent(BangBangCall.getContext(), Main.class);
		try{
			...
			BangBangCall.getContext().startActivity(intent);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
