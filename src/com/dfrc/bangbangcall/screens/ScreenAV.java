...
*/
package com.dfrc.bangbangcall.screens;
...
import com.dfrc.bangbangcall.BangBangCall;
import com.dfrc.bangbangcall.Engine;
import com.dfrc.bangbangcall.Main;
import com.dfrc.bangbangcall.R;
import com.dfrc.bangbangcall.Utils.DialerUtils;
import com.dfrc.bangbangcall.services.IScreenService;
...
public class ScreenAV extends BaseScreen{
	...
	protected void onStart() {
		...
		final KeyguardManager keyguardManager = BangBangCall.getKeyguardManager();
		...
		final PowerManager powerManager = BangBangCall.getPowerManager();
		...
		if(mProxSensor == null && !BangBangCall.isBuggyProximitySensor()){
	...
	public boolean back(){
		...
		if(ret){
			mScreenService.destroy(getScreenId());
		}
		return ret;
	}
		...
		public void run() {
			ScreenAV.this.runOnUiThread(new Runnable() {
				public void run() {
					...
					boolean gotoHome = (currentScreen != null && currentScreen.getScreenId() == getScreenId());
					if(gotoHome){
						mScreenService.show(ScreenHome.class);
					}
					mScreenService.destroy(getScreenId());
	...
	static class MyProxSensor implements SensorEventListener
	{
		...
		MyProxSensor(ScreenAV avScreen){
			mAVScreen = avScreen;
			mSensorManager = BangBangCall.getSensorManager();
		}
